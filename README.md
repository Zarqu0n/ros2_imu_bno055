...
**Description**: ROS2 driver for the BNO055 IMU using serial communication. Also it allows the configuration and calibration of the IMU
...
* Ubuntu with ROS2 (Humble, Iron, or later)
* Python 3
* Bosch IMU BNO055
...
ros2_imu_bno055 package depends on the following python/ros packages:

* pyserial
* rviz_imu_plugin (optional, for visualization)

## 4. Installation

Install ros2_imu_bno055 package

```
$ cd ~/ros2_ws/src
$ git clone https://github.com/RoboticArts/ros2_imu_bno055.git
```
...
```
$ cd ~/ros2_ws
$ colcon build --packages-select ros2_imu_bno055
$ source install/setup.bash
```

By default the standard ROS2 installation does not include any plugin to display the orientation of the IMUs in rviz. Therefore, you will need to install the **Imu plugin**. If you does not need to see the IMU in rviz you can skip this step.
...
```
$ cd ~/ros2_ws/src/ros2_imu_bno055
...
2. In a terminal, launch the following command to display the IMU in RVIZ2

```
$ ros2 launch ros2_imu_bno055 view_imu.launch.py
```

3. Move the IMU to view its orientation in RVIZ2.

<img src="utils/images/bno055_rviz.png" width="60%">


## 6. BNO055 ROS2 driver

1. Use the command below to start the driver in ROS2. This launch file is the same as the previous one but neither the tf nor rviz2 is published

```
$ ros2 launch ros2_imu_bno055 imu.launch.py serial_port:=/dev/ttyUSB0
```
...
| frequency | 50 | Frequency of reading the IMU and publication in ROS2. The maximum frequency reached by the IMU is 50 Hz |
...
**Note: Sensors not used in one mode of operation will turn off.** For example, if the IMU mode is selected, the magnetometer will not work and therefore its value cannot be published in ROS2. Also keep in mind that its orientation will be relative.
...
```
$ ros2 service call /imu/reset_device std_srvs/srv/Empty "{}"
```
...
```
$ ros2 service call /imu/calibration_status std_srvs/srv/Trigger "{}"
```

**7. Calibration load**

Every time the driver is launched automatically it will look for the calibration file called ```calibration```. If found, it will load the calibration into the IMU. If not found it will not load it and the IMU will use the internal factory calibration. This file is in the package directory.
...
9. If you are going to launch it from the terminal:

```
$ ros2 launch ros2_imu_bno055 imu.launch.py serial_port:=/dev/ttyUSB0 operation_mode:=IMU
```

## 7. IMU calibration

BNO055 IMU has an internal factory calibration, however, it is highly recommended to calibrate it. The node ```imu_calibration``` generates a calibration file for the accelerometer, gyroscope, and magnetometer. The generated file will be used by the bno055 ros2 driver to load it into the IMU. Remember that BNO055 does not have an EEPROM or flash memory to store the calibration so every time the IMU is turned on or reset, the calibration has to be loaded.
...
```
$ ros2 launch ros2_imu_bno055 imu_calibration.launch.py serial_port:=/dev/ttyUSB0 operation_mode:=NDOF_FMC_OFF
```
...
When the IMU is fully calibrated the node will finish and the calibration file will be automatically generated in the package directory which will be used by bno055 ros2 driver to load calibration data into the IMU. 
